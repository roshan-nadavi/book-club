...
function SpoilerMessage({
  msg, isOwn, myCurrentChapter, senderLabel, formattedTime,
}: {
  msg: PrivateMessage;
  isOwn: boolean;
  myCurrentChapter: number | null;
      ...
      <div className={`flex items-center gap-2 ${isOwn ? "flex-row-reverse" : "flex-row"}`}>
        ...
        {msg.spoiler_chapter !== null && (
          <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${
            isLocked ? "bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-400" : "bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-400"
          }`}>Ch.{msg.spoiler_chapter}+</span>
        )}
      </div>
      {isLocked ? (
        <div className={`max-w-[75%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed flex items-center gap-2 select-none ${isOwn ? "rounded-tr-sm" : "rounded-tl-sm"} bg-neutral-100 dark:bg-neutral-800 border border-dashed border-neutral-300 dark:border-neutral-600`}>
          ...
          <span className="text-neutral-400 dark:text-neutral-500 italic text-xs">Locked until chapter {msg.spoiler_chapter}</span>
        </div>
      ) : (
...
export default function PrivateConversationsView({
  bookId, groupId, currentUserId, currentUserUsername,
  myCurrentChapter, totalChapters, initialRooms, allGroupMembers,
}: Props) {
  ...
  // ---------------------------------------------------------------------------
  // SWR — only active when a room is selected; key is null otherwise so SWR
  // does nothing. Automatically pauses when the tab is hidden.
  // ---------------------------------------------------------------------------
  const { data: swrData, mutate } = useSWR<{ messages: PrivateMessage[] }>(
    selectedRoomId ? `/api/private-rooms/${selectedRoomId}/messages` : null,
    fetcher,
    {
      refreshInterval: REFRESH_INTERVAL_MS,
      revalidateOnFocus: false,
      keepPreviousData: false, // clear messages when switching rooms
    }
  );
  ...
  // Scroll to bottom when first loading a room
  useEffect(() => {
    if (swrData && isAtBottomRef.current) {
      messagesEndRef.current?.scrollIntoView({ behavior: "instant" });
    }
  }, [selectedRoomId, swrData]);
  ...
  useEffect(() => {
    ...
    function onMouseMove(e: MouseEvent) {
      ...
      setLeftWidth(Math.min(Math.max(((e.clientX - rect.left) / rect.width) * 100, 20), 60));
    }
  ...
  useEffect(() => {
    ...
    function onTouchMove(e: TouchEvent) {
      if (!containerRef.current) return;
      const rect = containerRef.current.getBoundingClientRect();
      setLeftWidth(Math.min(Math.max(((e.touches[0].clientX - rect.left) / rect.width) * 100, 20), 60));
    }
  ...
  async function handleSendMessage(e: React.FormEvent) {
    ...
    const optimisticMsg: PrivateMessage = {
      id: tempId, room_id: selectedRoomId,
      sender_id: currentUserId, sender_username: currentUserUsername,
      content, created_at: new Date().toISOString(), spoiler_chapter: spoilerChapterValue,
    };
  ...
  function openCreateModal() {
    setNewGroupName(""); setSelectedMemberIds(new Set([currentUserId])); setCreateError(null); setShowCreateModal(true);
  }

  function toggleMember(userId: string) {
    if (userId === currentUserId) return;
    setSelectedMemberIds((prev) => { const next = new Set(prev); next.has(userId) ? next.delete(userId) : next.add(userId); return next; });
  }
  ...
  async function handleCreateGroup(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/books/${bookId}/private-rooms`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ group_id: groupId, member_ids: [...selectedMemberIds], group_name: newGroupName.trim() || null }),
    });
    ...
    const newRoom: PrivateRoom = { id: data.roomId, book_id: bookId, group_name: newGroupName.trim() || null, created_at: new Date().toISOString(), members: newMembers };
    setRooms((prev) => [...prev, newRoom]);
    setCreating(false); setShowCreateModal(false);
  ...
  const isLoadingFirstFetch = selectedRoomId !== null && swrData === undefined;
      ...
      <div ref={containerRef} className="flex flex-1 overflow-hidden" style={{ userSelect: isDragging ? "none" : undefined }}>
        ...
        <div className="flex flex-col overflow-hidden border-r border-neutral-200 bg-background" style={{ width: `${leftWidth}%` }}>
          <div className="shrink-0 px-3 py-3 border-b border-neutral-200 bg-surface flex items-center justify-between gap-2">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-neutral-500 dark:text-neutral-400">Private Groups</h2>
            <button onClick={openCreateModal} className="shrink-0 rounded-lg bg-blue-600 px-2.5 py-1.5 text-xs font-semibold text-white hover:bg-blue-700 active:bg-blue-800 transition whitespace-nowrap">
              + Create New
            </button>
          </div>
                    ...
                    <li key={room.id}>
                      <button onClick={() => selectRoom(room.id)}
                        className={`w-full text-left px-4 py-3 transition flex flex-col gap-1.5 ${
                          isSelected ? "bg-blue-50 dark:bg-blue-950/40 border-l-2 border-blue-500" : "hover:bg-neutral-50 dark:hover:bg-neutral-800 border-l-2 border-transparent"
                        }`}>
                        <span className="text-sm font-semibold leading-snug truncate block text-black">{title}</span>
                        <span className="text-xs text-neutral-500 dark:text-neutral-400 leading-snug line-clamp-2">{memberPreview(room.members, 5)}</span>
                      </button>
                    </li>
                  );
                })}
          ...
          {selectedRoom ? (
            <>
              ...
              <div ref={scrollContainerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
                ...
                {!isLoadingFirstFetch && messages.length === 0 && (
                  <p className="text-sm text-neutral-400 dark:text-neutral-500 text-center py-8">No messages yet. Say something!</p>
                )}
                {messages.map((msg) => {
                  const isOwn = msg.sender_id === currentUserId;
                  return (
                    <SpoilerMessage key={msg.id} msg={msg} isOwn={isOwn} myCurrentChapter={myCurrentChapter}
                ...
                <div className="flex items-center gap-2">
                  ...
                  {spoilerEnabled && (
                    <>
                      ...
                      {spoilerChapter !== "" && (
                        <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-400">Ch.{spoilerChapter}+</span>
                      )}
                    </>
                  )}
                </div>
            ...
            <form onSubmit={handleCreateGroup} className="flex flex-col flex-1 min-h-0">
              <div className="px-5 pb-3">
                ...
                <input type="text" value={newGroupName} onChange={(e) => setNewGroupName(e.target.value)}
                  placeholder="e.g. Spoiler Chat" maxLength={80}
                    ...
                    return (
                      <li key={member.user_id}>
                        <label className={`flex items-center gap-3 px-3 py-2.5 cursor-pointer transition ${isMe ? "opacity-60 cursor-default" : "hover:bg-neutral-50 dark:hover:bg-neutral-800"}`}>
                          <input type="checkbox" checked={isChecked} onChange={() => toggleMember(member.user_id)} disabled={isMe}
                          ...
                          <span className="text-sm text-neutral-800 dark:text-neutral-200 truncate">
                            {member.username}{isMe && <span className="ml-1 text-xs text-neutral-400">(you)</span>}
                          </span>
                        </label>
                      </li>
                    );
              ...
              <div className="shrink-0 px-5 pb-5 flex gap-2">
                <button type="button" onClick={() => setShowCreateModal(false)} className="flex-1 rounded-lg border border-neutral-300 dark:border-neutral-700 px-3 py-2 text-xs font-medium text-neutral-700 dark:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-neutral-800 transition">Cancel</button>
          ...
          <div className="relative bg-white dark:bg-neutral-900 rounded-xl shadow-2xl border border-neutral-200 dark:border-neutral-700 w-full max-w-xs flex flex-col max-h-[70vh]">
            <div className="shrink-0 flex items-center justify-between px-5 pt-5 pb-3 border-b border-neutral-100 dark:border-neutral-800">
              <h3 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">
                Members <span className="ml-2 text-xs font-normal text-neutral-400">({selectedRoom.members.length})</span>
              </h3>
              ...
              {selectedRoom.members.map((member) => {
                ...
                return (
                  <li key={member.user_id} className="flex items-center gap-2.5 px-3 py-2.5">
                    ...
                    <span className="text-sm text-neutral-800 dark:text-neutral-200 truncate">
                      {member.username ?? "Unknown"}{isMe && <span className="ml-1 text-xs text-neutral-400">(you)</span>}
                    </span>
                  </li>
                );
              })}
