...
export default function GroupSplitView({
  ...
  function openAddBook() {
    setBookTitle(""); setBookAuthor(""); setBookChapters(""); setAddBookError(null); setShowAddBook(true);
  }
  ...
  async function handleAddBook(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/groups/${groupId}/books`, {
      ...
      body: JSON.stringify({ title: bookTitle.trim(), author: bookAuthor.trim() || null, total_chapters: bookChapters ? Number(bookChapters) : null }),
    });
    const data = await res.json();
    if (!res.ok) { setAddBookError(data.error ?? "Failed to add book."); setAddingBook(false); return; }
    setBooks((prev) => [...prev, { id: data.bookId, group_id: groupId, title: bookTitle.trim(), author: bookAuthor.trim() || null, total_chapters: bookChapters ? Number(bookChapters) : null, created_at: new Date().toISOString() }]);
    setAddingBook(false);
  ...
  async function handleSendMessage(e: React.FormEvent) {
    ...
    const optimisticMsg: Message = {
      id: tempId, group_id: groupId, book_id: null,
      sender_id: currentUserId, content,
  ...
  async function openMembersModal() { setShowMembersModal(true); await loadMembers(); }
  async function openKickModal() { setKickError(null); setConfirmKickTarget(null); setShowKickModal(true); await loadMembers(); }
  function requestKick(member: Member) { setKickError(null); setConfirmKickTarget(member); }

  async function confirmKick() {
    if (!confirmKickTarget) return;
    setKicking(true); setKickError(null);
    ...
    const data = await res.json();
    if (!res.ok) { setKickError(data.error ?? "Failed to kick member."); setKicking(false); setConfirmKickTarget(null); return; }
    setMembers((prev) => prev.filter((m) => m.user_id !== confirmKickTarget.user_id));
    if (data.newInviteCode) setInviteCode(data.newInviteCode);
    setKicking(false); setConfirmKickTarget(null);
  }

  function cancelKick() { setConfirmKickTarget(null); }
  ...
  return (
    <>
      <div ref={containerRef} className="flex flex-1 overflow-hidden" style={{ userSelect: isDragging ? "none" : undefined }}>
        ...
        <div className="flex flex-col overflow-hidden border-r border-neutral-200 bg-background" style={{ width: `${leftWidth}%` }}>
          <div className="shrink-0 px-4 py-3 border-b border-neutral-200 bg-surface">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-neutral-500 dark:text-neutral-400">Books</h2>
          </div>
          <div className="flex-1 overflow-y-auto">
            {books.length === 0 ? (
              <p className="px-4 py-6 text-sm text-neutral-400 dark:text-neutral-500">No books yet. Add one below.</p>
            ) : (
                ...
                {books.map((book) => (
                  <li key={book.id} className="relative group">
                    ...
                    >
                      ...
                      {book.author && <span className="text-xs text-neutral-500 dark:text-neutral-400">{book.author}</span>}
                      {book.total_chapters != null && <span className="text-xs text-neutral-400 dark:text-neutral-500">{book.total_chapters} chapters</span>}
                    </Link>
          ...
          <div className="shrink-0 px-4 py-3 border-t border-neutral-100 dark:border-neutral-800">
            <button onClick={openAddBook} className="w-1/4 min-w-[80px] rounded-lg bg-blue-600 px-3 py-2 text-xs font-semibold text-white hover:bg-blue-700 active:bg-blue-800 transition">
              + Add Book
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col flex-1 overflow-hidden bg-background">
          <div className="shrink-0 px-4 py-3 border-b border-neutral-200 bg-surface flex items-center justify-between gap-3">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-neutral-500 dark:text-neutral-400">Group Discussion</h2>
            <div className="flex items-center gap-2">
              <button onClick={openMembersModal} className="rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-800 px-3 py-1.5 text-xs font-medium text-neutral-700 dark:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-neutral-700 transition">
                List All Members
              </button>
              {isAdmin && (
                <button onClick={openKickModal} className="rounded-lg border border-red-300 dark:border-red-700 bg-white dark:bg-neutral-800 px-3 py-1.5 text-xs font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950 transition">
                  Kick Members
                </button>
              )}
            </div>
          ...
          <div ref={scrollContainerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
            {messages.length === 0 && (
              <p className="text-sm text-neutral-400 dark:text-neutral-500 text-center py-8">No messages yet. Say hello!</p>
            )}
            {messages.map((msg) => {
              ...
              return (
                <div key={msg.id} className={`flex flex-col gap-1 ${isOwn ? "items-end" : "items-start"}`}>
                  <div className={`flex items-center gap-2 ${isOwn ? "flex-row-reverse" : "flex-row"}`}>
                    <span className="text-[11px] text-neutral-400 dark:text-neutral-500 font-medium truncate max-w-[180px]">{senderLabel}</span>
                    <span className="text-[11px] text-neutral-400 dark:text-neutral-500">{formatTime(msg.created_at)}</span>
                  </div>
      ...
      {/* ADD BOOK MODAL */}
      {showAddBook && (
        <div className="fixed inset-0 z-50 flex items-center bg-black/40 dark:bg-black/60"
          onClick={(e) => { if (e.target === e.currentTarget) setShowAddBook(false); }}
          style={{ justifyContent: `${leftWidth / 2}%` }}>
          <div className="relative bg-white dark:bg-neutral-900 rounded-xl shadow-2xl border border-neutral-200 dark:border-neutral-700 p-5 flex flex-col gap-4" style={{ width: "min(300px, 90vw)" }}>
            <button onClick={() => setShowAddBook(false)} className="absolute top-3 right-3 text-neutral-400 hover:text-neutral-700 dark:hover:text-neutral-200 transition" aria-label="Close">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
            <h3 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100 pr-6">Add a Book</h3>
            <form onSubmit={handleAddBook} className="flex flex-col gap-3">
              <div>
                <label className="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">Title <span className="text-red-500">*</span></label>
                <input type="text" required value={bookTitle} onChange={(e) => setBookTitle(e.target.value)} placeholder="e.g. Dune"
              ...
              <div>
                <label className="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">Author <span className="text-red-500">*</span></label>
                <input type="text" required value={bookAuthor} onChange={(e) => setBookAuthor(e.target.value)} placeholder="e.g. Frank Herbert"
                  className="w-full rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-800 px-3 py-1.5 text-sm text-neutral-900 dark:text-neutral-100 placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                <label className="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">Number of Chapters <span className="text-red-500">*</span></label>
                <input type="number" required min={1} value={bookChapters} onChange={(e) => setBookChapters(e.target.value)} placeholder="e.g. 48"
                  className="w-full rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-800 px-3 py-1.5 text-sm text-neutral-900 dark:text-neutral-100 placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              {addBookError && <p className="text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded px-2 py-1.5">{addBookError}</p>}
              <div className="flex gap-2 pt-1">
                <button type="button" onClick={() => setShowAddBook(false)} className="flex-1 rounded-lg border border-neutral-300 dark:border-neutral-700 px-3 py-1.5 text-xs font-medium text-neutral-700 dark:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-neutral-800 transition">Cancel</button>
                <button type="submit" disabled={addingBook} className="flex-1 rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition">{addingBook ? "Adding…" : "Add Book"}</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="relative bg-white dark:bg-neutral-900 rounded-xl shadow-2xl border border-neutral-200 dark:border-neutral-700 w-full max-w-xs flex flex-col max-h-[70vh]">
            <div className="shrink-0 flex items-center justify-between px-5 pt-5 pb-3 border-b border-neutral-100 dark:border-neutral-800">
              <h3 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">
                Members {!membersLoading && <span className="ml-2 text-xs font-normal text-neutral-400">({members.length})</span>}
              </h3>
            ...
            <div className="flex-1 overflow-y-auto">
              {membersLoading && <p className="px-5 py-6 text-sm text-neutral-400 text-center">Loading…</p>}
              {membersError && <p className="px-5 py-6 text-sm text-red-500 text-center">{membersError}</p>}
                    ...
                    return (
                      <li key={member.user_id} className="flex items-center gap-2.5 px-3 py-2.5">
                        <div className="w-7 h-7 rounded-full bg-neutral-200 dark:bg-neutral-700 flex items-center justify-center shrink-0">
                          <span className="text-xs font-semibold text-neutral-600 dark:text-neutral-300 uppercase">{displayName[0]}</span>
                        </div>
                        <span className="text-sm text-neutral-800 dark:text-neutral-200 truncate">
                          {displayName}{isMe && <span className="ml-1 text-xs text-neutral-400">(you)</span>}
                        </span>
                        {isMemberAdmin && <span className="ml-auto shrink-0 text-xs font-semibold text-red-500 dark:text-red-400">(admin)</span>}
                      </li>
                    );
                  })}
                </ul>
      ...
      {showKickModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 dark:bg-black/70 px-4"
          onClick={(e) => { if (e.target === e.currentTarget) { setShowKickModal(false); setKickError(null); } }}>
          <div className="relative bg-white dark:bg-neutral-900 rounded-xl shadow-2xl border border-neutral-200 dark:border-neutral-700 w-full max-w-xs flex flex-col max-h-[70vh]">
            <div className="shrink-0 flex items-center justify-between px-5 pt-5 pb-3 border-b border-neutral-100 dark:border-neutral-800">
              <h3 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">Kick Members</h3>
              <button onClick={() => { setShowKickModal(false); setKickError(null); }} className="text-neutral-400 hover:text-neutral-700 dark:hover:text-neutral-200 transition" aria-label="Close">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            {kickError && (
              <div className="mx-4 mt-3">
                <p className="text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded px-3 py-2">{kickError}</p>
              </div>
            )}
            <div className="flex-1 overflow-y-auto">
              {membersLoading && <p className="px-5 py-6 text-sm text-neutral-400 text-center">Loading…</p>}
              ...
              {!membersLoading && !membersError && (
                <ul className="divide-y divide-neutral-100 dark:divide-neutral-800 px-2 py-2">
                  {members.filter((m) => m.user_id !== adminId).map((member) => {
                    ...
                    return (
                      <li key={member.user_id} className="flex items-center gap-2.5 px-3 py-2.5">
                        ...
                        <button onClick={() => requestKick(member)}
                  ...
                  {members.filter((m) => m.user_id !== adminId).length === 0 && (
                    <li className="px-4 py-6 text-center text-sm text-neutral-400 dark:text-neutral-500">No other members to kick.</li>
                  )}
                </ul>
              )}
            </div>
      ...
      {confirmKickTarget && (
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/60 dark:bg-black/75 px-4">
          <div className="bg-white dark:bg-neutral-900 rounded-xl shadow-2xl border border-neutral-200 dark:border-neutral-700 w-full max-w-xs p-6 flex flex-col gap-4">
            <p className="text-sm text-neutral-800 dark:text-neutral-200 text-center leading-relaxed">
              Are you sure you want to kick out <span className="font-semibold">{confirmKickTarget.username ?? "this member"}</span>?
            </p>
            <div className="flex gap-3">
              <button onClick={cancelKick} disabled={kicking} className="flex-1 rounded-lg border border-neutral-300 dark:border-neutral-700 px-3 py-2 text-sm font-medium text-neutral-700 dark:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-neutral-800 disabled:opacity-50 transition">No</button>
              <button onClick={confirmKick} disabled={kicking} className="flex-1 rounded-lg bg-red-600 px-3 py-2 text-sm font-semibold text-white hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition">{kicking ? "Kicking…" : "Yes"}</button>
            </div>
          </div>
        </div>
      )}
